...
Click anywhere on a room (floor, walls or its label) to open its stats, the vault for the roster, or the dock. Click a character to follow them: the camera tracks them and the panel says what they are doing and whether it is real. Drag to pan, scroll or pinch to zoom, double-click a room to fly in, `1`-`9` jump to a room, `C` customizes the world, `R` replays the last 24 hours of real events (it also plays on its own when the world is left alone), `M` turns sound on, `Esc` returns to the overview, `?` opens the field guide. `V` opens a room's close-up, `[` and `]` step room to room, `T` starts the tour. Add `?hour=22` to any page to see the world at night. Up close the folk are drawn like village-game troops: half again as big, smoothed from their pixel sprites, bouncing as they walk, with level badges and outlined names; a room you click fills the screen (the area view). The whole world is smoothed the same way (upscaled once per change, in a background worker, whenever the screen has the pixels to show it), everyone who works stands on a ring of their own colour with a rim to match and a name over their head, and each room has a medallion on its floor saying what it is. Add `?hd=0` to go back to plain pixel art.
...
src/station.html  the pixel-art station: canvas renderer (lighting, bloom, depth-aware crew, pathfinding, camera), the room close-up, the tour, field help, the phone layout and game HUD; no assets, scenery labelled as scenery
